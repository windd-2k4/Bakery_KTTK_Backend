...
@Controller('admin/api/v1/pastries')
export class AdminPastryController {
  ...
  async getAll(@Query('category') category?: string): Promise<ApiResponse<PastryResponseDto[]>> {
    const data = category ? await this.pastryService.findByCategory(category) : await this.pastryService.findAllAdmin();
    return new ApiResponse(200, 'Successfully!', data);
  }

  @Get('search')
  async search(@Query('keyword') keyword: string): Promise<ApiResponse<PastryResponseDto[]>> {
    const data = await this.pastryService.searchAdmin(keyword);
    return new ApiResponse(200, 'Successfully!', data);
  }
}